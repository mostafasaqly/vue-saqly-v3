export default {
  ...
  lessons: [
    ...
    "هيكل المشروع",
    "useTaskStore — Pinia Store للمهام",
    "useTaskFilter — Composable للفلترة والبحث",
    "TaskItem Component",
    "AddTaskForm Component",
    "App.vue — تجميع كل شيء",
  ...
  lessonsEn: [
    ...
    "Project Structure",
    "useTaskStore — Pinia Store for Tasks",
    "useTaskFilter — Filtering & Search Composable",
    "TaskItem Component",
    "AddTaskForm Component",
    "App.vue — Putting It All Together",
    "localStorage Persistence",
    "Form Validation",
    "Skills Practiced in This Project",
  ],
  intro: "نبني تطبيق إدارة مهام كامل — يجمع كل ما تعلمناه: Composition API، Pinia، Composables، Transitions، وlocalStorage.",
  introEn: "Build a complete task manager app — combining everything learned: Composition API, Pinia, Composables, Transitions, and localStorage.",
  content: [
    { type: "heading", text: "🗂️ نظرة عامة على المشروع" },
    { type: "paragraph", text: "تطبيق Task Manager كامل بالميزات التالية:" },
    { type: "list", items: [
      "إضافة مهام مع التحقق من النموذج",
      "تحديد المهام كمُنجَزة بضغطة واحدة",
      "تعديل المهام مباشرة (inline editing)",
      "حذف مع رسالة تأكيد",
      "فلترة: الكل / نشطة / منجزة",
      "بحث نصي فوري",
      "ترتيب حسب التاريخ / الأبجدية",
      "حفظ تلقائي في localStorage",
      "Transitions عند الإضافة والحذف",
    ]},
...
└── App.vue               # صفحة الرئيسية تجمع كل شيء` },
    { type: "heading", text: "useTaskStore — Pinia Store" },
...
export const useTaskStore = defineStore('tasks', () => {
  ...
  return {
    ...
    { type: "heading", text: "useTaskFilter — Composable للفلترة" },
    { type: "code", code: `// composables/useTaskFilter.js
...
<template>
  <div class="task-item" :class="{ 'task-item--done': task.done, ['priority-' + task.priority]: true }">
    ...
    { type: "heading", text: "AddTaskForm Component" },
    { type: "code", code: `<!-- AddTaskForm.vue -->
<script setup>
import { ref } from 'vue'
import { useTaskStore } from '@/stores/useTaskStore'
...
<template>
  <form @submit.prevent="submit" class="add-form">
    ...
    { type: "heading", text: "App.vue — تجميع كل شيء" },
...
<template>
  <div class="app">
    ...
    { type: "list", items: [
      "Pinia Setup Store — state وgetters وactions",
      "watch مع deep: true لـ persistence تلقائي في localStorage",
      "Composable — useTaskFilter يُستخدم في الـ App.vue",
      "Transition وTransitionGroup للرسوم المتحركة",
      "Form Validation — error messages مع Transition",
      "Inline Editing مع keyup.enter وkeyup.escape",
      "computed للاشتقاق الديناميكي (filtered list)",
      "storeToRefs للحفاظ على reactivity",
    ]},
    { type: "heading", text: "✅ مراجعة المشروع" },
    { type: "qa", question: "لماذا نستخدم Pinia بدلاً من props/emits لهذا المشروع؟", answer: "لأن البيانات تحتاجها عدة components في مستويات مختلفة — بدون Pinia ستحتاج prop drilling أو event bus. Pinia يضع البيانات في مكان مركزي يصله أي component مباشرة." },
    { type: "qa", question: "لماذا فصلنا منطق الفلترة في Composable منفصل؟", answer: "لفصل المسؤوليات — الـ store يتعامل مع البيانات (CRUD)، الـ Composable يتعامل مع عرض البيانات (filter/search/sort). يُسهّل الاختبار المستقل لكل منطق." },
    { type: "qa", question: "كيف نحفظ المهام تلقائياً في localStorage؟", answer: "نستخدم watch مع { deep: true } على الـ tasks ref. عند كل تغيير (إضافة/تعديل/حذف)، يُنفَّذ الـ watcher ويُحدّث localStorage. نحمّل البيانات عند الإنشاء: ref(JSON.parse(localStorage.getItem('vue-tasks') || '[]'))." },
    { type: "cta", text: "هل تريد تدريباً مخصصاً على Vue وبناء مشاريع احترافية؟", linkLabel: "تواصل معنا →", link: "https://saqly.com/individual-training" },
  ],
  contentEn: [
    ...
    { type: "list", items: [
      ...
      "Inline task editing (dblclick)",
      "Delete with confirmation",
      "Filter: all / active / done",
      "Instant text search",
      "Sort by date or alphabetically",
      "Auto-save to localStorage",
      "Transitions on add/remove",
    ]},
    { type: "heading", text: "useTaskStore" },
    { type: "code", code: `// stores/useTaskStore.js
import { defineStore } from 'pinia'
import { ref, computed, watch } from 'vue'
...
export const useTaskStore = defineStore('tasks', () => {
  ...
  return { tasks, activeCount, doneCount, addTask, toggleTask, editTask, deleteTask }
    ...
    { type: "heading", text: "useTaskFilter Composable" },
    { type: "code", code: `// composables/useTaskFilter.js
import { ref, computed } from 'vue'
...
export function useTaskFilter(tasks) {
  ...
  return { filter, search, sortBy, filtered }
    ...
    { type: "list", items: [
      "Pinia Setup Store — state, getters, actions",
      "watch with deep: true for auto-persistence",
      "Composable — useTaskFilter for filtering logic",
      "Transition & TransitionGroup for animations",
      "Form validation with error Transition",
      "Inline editing with keyup.enter/escape",
    ]},
    { type: "heading", text: "✅ Project Review" },
    { type: "qa", question: "Why use Pinia instead of props/emits?", answer: "Because data is needed by multiple components at different levels. Without Pinia you'd need prop drilling or an event bus. Pinia stores data centrally, accessible by any component directly." },
    { type: "qa", question: "Why separate filter logic into a Composable?", answer: "Separation of concerns — the store handles data (CRUD), the Composable handles presentation (filter/search/sort). Each can be tested independently." },
    { type: "cta", text: "Want personalized Vue training and project-building sessions?", linkLabel: "Contact Us →", link: "https://saqly.com/individual-training" },
  ],
};
