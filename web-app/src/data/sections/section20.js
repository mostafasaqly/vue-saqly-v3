export default {
  id: 20,
  title: "المشروع الثالث: تطبيق E-Commerce مصغّر",
  titleEn: "Project 3: Mini E-Commerce App",
  level: "تطبيق عملي",
  levelEn: "Hands-on Project",
  lessons: [
    "نظرة عامة على المشروع والميزات",
    "هيكل المشروع والـ Router",
    "useCartStore — Pinia للسلة",
    "Cart icon مع badge في الـ Navbar",
    "ProductCard مع زر Add to Cart",
    "CartView — صفحة السلة الكاملة",
    "Checkout Form مع Validation",
    "Order Success Page",
    "استمرارية السلة في localStorage",
    "مهارات تطبّقها في هذا المشروع",
  ],
  lessonsEn: [
    "Project Overview & Features",
    "Project Structure & Router",
    "useCartStore — Pinia for Cart",
    "Cart Icon with Badge in Navbar",
    "ProductCard with Add to Cart",
    "CartView — Full Cart Page",
    "Checkout Form with Validation",
    "Order Success Page",
    "Cart Persistence with localStorage",
    "Skills Practiced in This Project",
  ],
  intro: "المشروع الثالث والأكبر — متجر إلكتروني مصغّر يجمع كل مهارات الكورس: Vue Router + Pinia + Axios + Composition API.",
  introEn: "The biggest project — a mini e-commerce app combining all course skills: Vue Router + Pinia + Axios + Composition API.",
  content: [
    { type: "heading", text: "🛒 نظرة عامة على المشروع" },
    { type: "list", items: [
      "صفحة منتجات مع بحث وفلترة بالفئة",
      "صفحة تفاصيل المنتج مع تحديد الكمية",
      "سلة تسوق كاملة مع تحديث الكمية والحذف",
      "نموذج Checkout مع validation كامل",
      "ملخص الطلب مع حساب الضريبة",
      "صفحة Order Success بعد الطلب",
...
└── views/
    ...
    └── OrderSuccessView.vue # تأكيد الطلب` },
...
export default createRouter({
  ...
  routes: [
    ...
    { type: "heading", text: "useCartStore — Pinia الكامل" },
...
export const useCartStore = defineStore('cart', () => {
  ...
  return {
    ...
    { type: "heading", text: "CartIcon في الـ Navbar" },
    { type: "code", code: `<!-- components/CartIcon.vue -->
<script setup>
...
@keyframes bounce {
  ...
  50% { transform: scale(1.4); }
    ...
    { type: "heading", text: "CartView — صفحة السلة" },
    { type: "code", code: `<!-- views/CartView.vue -->
<script setup>
import { useCartStore } from '@/stores/useCartStore'
import { storeToRefs } from 'pinia'
...
<template>
  <div class="cart-page">
    ...
    { type: "heading", text: "Checkout Form مع Validation" },
    { type: "code", code: `<!-- views/CheckoutView.vue -->
<script setup>
...
<template>
  <form class="checkout-form" @submit.prevent="submitOrder">
    ...
    { type: "list", items: [
      "Pinia — useCartStore: computed للمجاميع، watch deep للـ localStorage persistence",
      "Navigation Guard — منع الوصول للـ checkout بسلة فارغة",
      "Form Validation — rules object، blur-based validation، error display",
      "Transition/TransitionGroup — bounce على badge، animation عند حذف عنصر",
      "storeToRefs — destructuring reactivity من الـ store",
      "computed مشتقة — tax، shipping، total من subtotal",
      "Vue Router — programmatic navigation بعد إتمام الطلب",
    ]},
    { type: "heading", text: "✅ مراجعة المشروع" },
    { type: "qa", question: "لماذا نستخدم watch مع deep: true لحفظ السلة في localStorage؟", answer: "لأن items هو مصفوفة من كائنات. watch العادي يكتشف فقط إعادة تعيين المصفوفة نفسها. deep: true يجعل Vue يراقب التغييرات الداخلية — تحديث qty، إضافة عنصر جديد، حذف عنصر." },
    { type: "qa", question: "لماذا نستخدم Navigation Guard للـ /checkout route؟", answer: "لمنع المستخدم من الوصول لصفحة الدفع مباشرة عبر URL وهو لا يملك عناصر في السلة. الـ guard يفحص cart.items.length — إذا كان صفراً يُعيد توجيهه للصفحة الرئيسية." },
    { type: "qa", question: "ما فائدة تفصيل الـ Validation إلى rules object؟", answer: "يجعل قواعد التحقق مجمّعة وقابلة للقراءة. بدلاً من if/else متكررة، نستخدم Object.keys(rules).forEach() للتحقق من كل الحقول في دالة واحدة. يسهّل إضافة حقول جديدة ومعدّل القواعد." },
    { type: "cta", text: "أتقنت بناء مشاريع Vue احترافية — للارتقاء إلى المستوى التالي:", linkLabel: "تدريب مخصص →", link: "https://saqly.com/individual-training" },
  ],
  contentEn: [
...
export const useCartStore = defineStore('cart', () => {
  ...
  return { items, subtotal, tax, shipping, total, itemCount, addToCart, updateQty, removeItem, clearCart }
    ...
    { type: "heading", text: "Checkout Validation" },
    { type: "code", code: `const rules = {
...
function validateAll() {
  ...
  return !Object.values(errors).some(Boolean)
    ...
    { type: "list", items: [
      "Pinia — computed totals, deep watch for localStorage",
      "Navigation Guard — prevent checkout with empty cart",
      "Form Validation — rules object, blur-based validation",
      "Transition — bounce on badge, animation on cart item removal",
      "storeToRefs — reactive destructuring from store",
      "Programmatic navigation after order success",
    ]},
    { type: "heading", text: "✅ Project Review" },
    { type: "qa", question: "Why deep: true when watching the cart for localStorage?", answer: "Because items is an array of objects. A regular watch only detects full reassignment. deep: true makes Vue track internal changes — updating qty, adding or removing items inside the array." },
    { type: "qa", question: "Why use a Navigation Guard for the /checkout route?", answer: "To prevent users from accessing the checkout page directly via URL when they have no items in their cart. The guard checks cart.items.length — if zero, it redirects to the home page." },
    { type: "cta", text: "Ready to take your Vue skills to the next level?", linkLabel: "Individual Training →", link: "https://saqly.com/individual-training" },
  ],
};
